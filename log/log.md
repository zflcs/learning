...
- 不必使用系统调用来读取文件的符号表了，直接读取 GOT 表，或者称之为接口表？？？尽管是同一份代码，但是内核和用户进程的 EXECUTOR 是不同的，但创建的协程的id是统一的（**id 是直接使用的 static，则是直接在共享库的数据段中，EXECUTOR 是 lazy_static，所以它是在各自的堆中**）
