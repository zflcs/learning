...
- [x] 多线程没有唤醒是因为 gc 任务被唤醒是，其 on_cpu 字段为 true，因此进入了死循环，后续再无法唤醒
  - [x] 原本的 axtask 的实现中，清除前一个任务的 on_cpu 字段是会实时的获取到当前 CPU 上的前一个任务，而 vsched 中进行切换时，因为是被动的获取 percpu 字段，在切换前后不会变化，因此，一旦任务在不同 CPU 之间迁移，从一个 CPU 切换到另一个 CPU 时，因为 vsched 内部使用的 percpu 数据仍然为之前记录的，因此，会错误的清除前一个任务的 on_cpu 字段
  - [x] 最好的解决办法就是能够实时获取当前的 cpu_id，把 cpu_id 符号相对于 percpu 段的偏移记录下来，然后直接根据寄存器来获取 cpu_id
  - [x] 在其他的架构下，编译 vsched.so 库时会直接提示工具链不支持，需要将修改编译参数才可以，后续继续改进
- [x] 修改专利交底书
- [x] 写学期总结
...
- [x] 修改了 idle task 的栈大小后，fifo 调度器可以在多核下正常工作
  - [x] cfs 和 rr 在单核情况下工作还有 bug，修改 scheduler 中定义的字段顺序之后，能够正常运行了，
  - [x] 多核情况下偶尔出现运行没有唤醒任务的情况

## 20250722

- [x] 更新 TaskInner 数据结构定义
  - [x] 通过 core::mem::transmute 和声明外部函数的方式解决字段的依赖问题，保证不会对其他的模块产生影响
  - [x] cfs 和 rr 调度器都存在问题，fifo 没有问题，还需要进一步查找原因

## 20250721

- [x] 尝试解决 vsched-axtask 的 cfs 出现的 bug
  - [x] 单步调试时，vsched 中的 jalr 指令是正常跳转的，但是 qemu 的日志中没有这些指令的执行记录，导致调试找不到原因
  - [x] 潜在的问题：vsched 内部获取 percpu 时，没有地址对齐
  - [x] 使用 cfs 时，在进行切换时，寄存器错误了，还不知道具体的原因
  - [x] 尝试逐个排除错误
    - [x] 替换 scheduler 使用的底层数据结构，可以正常工作，证明与就绪队列的底层数据结构无关
    - [x] 单步调试时发现使用 cfs 调度器时，对栈指针进行操作时栈溢出了，修改了栈的大小
- [x] SMP 模式下仍然存在问题，尝试解决

## 20250720

- [x] 尝试解决 vsched-axtask 的 cfs 出现的 bug
  - [x] qemu 日志中显示指针乱飞，很多 jalr 指令没有执行，莫名其妙跳转到非法指令
  - [x] 去掉了 AtomicCell 后，没有出现奇怪的跳转了，但是 jalr 指令会直接跳转至 unimp 指令，或者直接不执行
  - [x] 还没有找到原因

## 20250719

- [x] 尝试解决 vsched-axtask 的 preempt 出现的 bug
  - [x] 因为在抢占的过程中，要获取当前的 CPU_ID，但是读取 CPU_ID 时，又要禁止抢占，因此出现了递归，所以才出现了指针乱飞和嵌套的现象
  - [x] Round-robin 调度可以正常工作了，但是 cfs 调度仍然存在问题，prev_task 使用了 weak_clone，但是没有及时 upgrade，这里的若引用计数是错误的
  - [ ] 使用 rr 时，只 clone 了一次，但是使用 cfs 时，clone 了三次，并且在 clone 方法中打印日志时会出现死锁
...
- [x] 尝试解决 vsched-axtask 的 preempt 出现的 bug
  - [x] 任务控制块的字段存在问题，使用 task_ext() 访问 TaskExt 时，出现了访存错误，数据结构没有问题
  - [x] 指针跑飞了
- [x] 组会
...
- [x] 将 vsched 移植到 Arceos 中
  - [x] 解决 block 时出现的 bug
    - [x] 地址映射时与设备的 MMIO 地址冲突了
    - [x] task_entry 和 coroutine_schedule 函数中没有使能中断，所以 idle 任务一直在等待中断
  - [x] 使能 SMP 时，支持 migrate 操作
  - [x] 在 cfs 调度器下，get_data_base 的位置变化了，出现了问题，导致获取的数据段起始位置错误
  - [x] current_may_uninit 存在问题，导致启动时没有输出，增加新的判断逻辑，并且对 kernel_guard 的实现也存在影响
  - [x] 解决编译的问题，避免手动复制共享库文件到 vsched-axtask 目录中

## 20250714

- [x] 将 vsched 移植到 Arceos 中
  - [x] 解决代码报错，可以进入 init_vsched 函数，但是因为一些 RQ_CAP 这些变量导致了出错
  - [x] 声明了 RQ_CAP 后，能够正常初始化调度器了
  - [x] current_may_uninit 中使用的 ptr_eq 存在问题，导致退出时出现问题，去掉了 ptr_eq 之后正常了
